...
const TestEvents = {
  LoanDueDate: require('./events/loan-due-date-event.json')
}
...
// Module under test
const LoanUpdated = rewire('../../src/loan-due-date/handler')

// Test data
const LoanDueDateEvent = require('./events/loan-due-date-event.json')
...
describe('loan updated lambda tests', () => {
  ...
  describe('SNS event tests', () => {
    it('should callback with a success message if the event is valid', (done) => {
      ...
      LoanUpdated.handle(TestEvents.LoanDueDate, null, (err, data) => {
        should.not.exist(err)
        data.should.equal('Loan 18263808770001221 successfully updated with event LOAN_DUE_DATE')
        done()
      })
    })
    ...
    it('should be rejected with an error if the event type is not LOAN_DUE_DATE', (done) => {
      const testEvent = {
        Records: [{
          Sns: {
  ...
  describe('end to end tests', () => {
    ...
    it('should call DynamoDB put correctly for the Users table', (done) => {
      ...
      LoanUpdated.handle(LoanDueDateEvent, {}, (err, data) => {
        should.not.exist(err)
